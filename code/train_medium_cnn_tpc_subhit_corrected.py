...
MODEL_DIR = Path("models_medium_cnn_corrected")
PLOT_DIR = Path("plots_medium_cnn_corrected")
WEIGHT_DIR = Path("weights_medium_cnn_corrected")
...
model = tf.keras.Sequential([
    ...
    tf.keras.layers.Conv2D(8, (3, 7), padding="same", activation="relu", name="conv1"),
    tf.keras.layers.MaxPooling2D((2, 2), name="pool1"),

    tf.keras.layers.Conv2D(16, (3, 11), padding="same", activation="relu", name="conv2"),
    tf.keras.layers.MaxPooling2D((2, 4), name="pool2"),

    tf.keras.layers.Conv2D(24, (3, 11), padding="same", activation="relu", name="conv3"),
    tf.keras.layers.MaxPooling2D((1, 4), name="pool3"),
...
model.compile(
    ...
    metrics=[
        tf.keras.metrics.BinaryAccuracy(name="acc_at_0p5", threshold=0.5),
        tf.keras.metrics.AUC(name="auc"),
        tf.keras.metrics.Precision(name="precision_at_0p5", thresholds=0.5),
        tf.keras.metrics.Recall(name="recall_at_0p5", thresholds=0.5),
    ],
)
...
checkpoint = tf.keras.callbacks.ModelCheckpoint(
    MODEL_DIR / "medium_cnn_best_by_val_auc.h5",
    monitor="val_auc",
    mode="max",
    save_best_only=True,
    verbose=1,
...
model.save(MODEL_DIR / "medium_cnn_final.h5")
...
plot_history_metric("auc", "AUC", "auc_curve_correct_metric.png")
plot_history_metric("acc_at_0p5", "Accuracy at threshold 0.5", "accuracy_at_0p5_misleading.png")
plot_history_metric("precision_at_0p5", "Precision at threshold 0.5", "precision_at_0p5.png")
plot_history_metric("recall_at_0p5", "Recall at threshold 0.5", "recall_at_0p5.png")
...
plot_threshold_scan(
    val_rows,
    "validation_threshold_scan_right_plot.png",
...
# ============================================================
# Test evaluation using validation-chosen threshold
# ============================================================
...
# ============================================================
# Score distributions with threshold line
# ============================================================
...
with open(MODEL_DIR / "test_predictions.csv", "w") as f:
    f.write("event_index,true_label,score,pred_0p50,pred_val_chosen,pred_test_best_f1\n")

    test_best_thr = test_best_f1["threshold"]

    for i, (yt, score) in enumerate(zip(y_test, test_scores)):
        pred_05 = int(score >= 0.5)
        pred_val = int(score >= chosen_threshold)
        pred_test_best = int(score >= test_best_thr)
        f.write(f"{i},{int(yt)},{score:.8f},{pred_05},{pred_val},{pred_test_best}\n")
