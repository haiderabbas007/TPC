#!/usr/bin/env python3
"""
train_cnn_hls_tiny_manual_prune_80_180.py

Robust FPGA-friendly training script for the 20x100 crop.

Why this version:
  - Avoids QKeras training incompatibilities in Keras 3.
  - Uses a much smaller normal Keras CNN.
  - Applies manual magnitude pruning.
  - Fine-tunes while keeping pruned weights exactly zero.
  - Saves a simple .h5 model that hls4ml can convert reliably.
  - Quantization should be applied in the hls4ml conversion config
    using fixed-point precisions such as ap_fixed<8,3> or ap_fixed<6,2>.
...
MODEL_DIR = Path("models_hls_tiny_manual_prune_cnn_80_180")
PLOT_DIR = Path("plots_hls_tiny_manual_prune_cnn_80_180")
WEIGHT_DIR = Path("weights_hls_tiny_manual_prune_cnn_80_180")
...
# If accuracy is poor, lower to 0.50. If HLS is still too heavy, raise to 0.80.
...
if X_train.shape[1:] != INPUT_SHAPE:
    raise ValueError(
        ...
        "Check that out_sub_80_180 is really the 80:180 crop."
    )
...
# ============================================================
# Tiny HLS-friendly CNN
# ============================================================

def build_tiny_model():
    """
    Much smaller than your earlier model.

    Earlier:
      Conv2D(4) -> Conv2D(8) -> Dense(8)

    This:
      Conv2D(2) -> Conv2D(4) -> Dense(4)

    hls4ml can later quantize the trained weights/activations through
    config precision settings.
    """
    model = tf.keras.Sequential(
        [
            ...
            tf.keras.layers.Dense(4, activation="relu", use_bias=True, name="dense1"),

            # Linear output logit for HLS.
            tf.keras.layers.Dense(1, activation="linear", use_bias=True, name="output"),
        ],
        name="hls_tiny_manual_prune_cnn_80_180",
    )
    return model
...
with open(MODEL_DIR / "tiny_model_summary.txt", "w") as f:
...
PRUNABLE_LAYER_TYPES = (tf.keras.layers.Conv2D, tf.keras.layers.Dense)

def get_prunable_weight_indices(layer):
    # For Conv2D/Dense: index 0 = kernel, index 1 = bias.
    # Prune kernels only. Biases are tiny and useful.
        ...
        for idx, arr in enumerate(weights):
            if idx in get_prunable_weight_indices(layer):
                ...
                if flat_abs.size == 0:
                    ...
                    # Keep at least one weight in very small layers.
                    if np.sum(mask) == 0:
                        keep_idx = np.argmax(flat_abs)
                        mask_flat = np.zeros_like(flat_abs, dtype=np.float32)
...
def apply_pruning_masks(model, masks):
    for layer in model.layers:
        ...
        for arr, mask in zip(weights, masks[layer.name]):
            masked_weights.append(arr * mask)

        layer.set_weights(masked_weights)
...
print("\n[+] Fine-tuning pruned model while keeping pruned weights zero...\n")
...
# ============================================================
# Save HLS-ready Keras model
# ============================================================

model.save(MODEL_DIR / "hls_tiny_manual_prune_cnn_80_180.h5")
model.save(MODEL_DIR / "hls_tiny_manual_prune_cnn_80_180.keras")
...
def plot_history_metric(key, ylabel, filename):
    ...
    warmup_len = len(history_warmup.history.get(key, []))
    if warmup_len > 0:
        plt.axvline(warmup_len - 1, linestyle="--", label="manual prune point")
    plt.xlabel("Epoch")
    plt.ylabel(ylabel)
    plt.title(f"Tiny Manual-Pruned HLS CNN 80-180 {ylabel}")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
...
all_weights_path = WEIGHT_DIR / "hls_tiny_manual_prune_cnn_80_180_all_weights.txt"

with open(all_weights_path, "w") as f_all:
    f_all.write("HLS Tiny Manual-Pruned CNN 80-180 Weights\n")
    f_all.write("========================================\n\n")
...
with open(MODEL_DIR / "hls4ml_precision_suggestion.txt", "w") as f:
    f.write("Suggested hls4ml quantization settings\n")
    f.write("=====================================\n\n")
    f.write("Start with:\n")
    f.write("  default precision: ap_fixed<8,3>\n")
    f.write("  weight precision : ap_fixed<8,2>\n")
    f.write("  bias precision   : ap_fixed<8,2>\n")
    f.write("  result precision : ap_fixed<10,4>\n\n")
    f.write("If accuracy after hls4ml conversion is poor, loosen to:\n")
    f.write("  default precision: ap_fixed<10,4>\n")
    f.write("  weight precision : ap_fixed<10,3>\n")
    f.write("  result precision : ap_fixed<12,5>\n\n")
    f.write("If HLS is still too heavy, tighten to:\n")
    f.write("  default precision: ap_fixed<6,2>\n")
    f.write("  weight precision : ap_fixed<6,1>\n")
    f.write("  result precision : ap_fixed<8,3>\n")


with open(MODEL_DIR / "final_summary.txt", "w") as f:
    f.write("Final Summary\n")
    f.write("=============\n\n")
    f.write("Model: HLS tiny manual-pruned CNN 80-180 crop\n")
    f.write("Input shape: 20x100x1\n")
    f.write("Crop: original columns 80:180\n\n")
    f.write("Architecture:\n")
    f.write("Conv2D(2, 3x5) -> ReLU -> MaxPool(2x2)\n")
    f.write("Conv2D(4, 3x7) -> ReLU -> MaxPool(2x4)\n")
    f.write("GlobalAveragePooling2D -> Dense(4) -> ReLU -> Dense(1 linear logit)\n\n")
...
print("\n[+] Done.")
print(f"[+] HLS-ready Keras model:")
print(f"    {MODEL_DIR / 'hls_tiny_manual_prune_cnn_80_180.h5'}")
print(f"[+] Precision suggestion:")
print(f"    {MODEL_DIR / 'hls4ml_precision_suggestion.txt'}")
