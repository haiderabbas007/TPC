...
MODEL_PATH = "models_hls_light_cnn_80_180/hls_light_cnn_80_180_final.h5"
OUTPUT_DIR = "hls4ml_light_cnn_80_180"
...
model = tf.keras.Model(inputs=new_input, outputs=x, name="hls_light_cnn_safe")
...
for layer_name in config["LayerName"]:
    config["LayerName"][layer_name]["Precision"] = "ap_fixed<6,2>"
    config["LayerName"][layer_name]["ReuseFactor"] = 16

for layer_name in config["LayerName"]:
    lname = layer_name.lower()
    if "softmax" in lname or "output" in lname or "dense" in lname:
        config["LayerName"][layer_name]["Precision"] = "ap_fixed<8,3>"
