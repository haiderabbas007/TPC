...
MODEL_DIR = Path("models_hls_light_cnn_80_180")
PLOT_DIR = Path("plots_hls_light_cnn_80_180")
WEIGHT_DIR = Path("weights_hls_light_cnn_80_180")
...
# ============================================================
# HLS-light CNN for cropped 20x100 input
# ============================================================

model = tf.keras.Sequential([
    tf.keras.layers.Input(shape=(20, 100, 1), name="input_layer"),
    ...
    # Linear output for HLS. BinaryCrossentropy uses from_logits=True.
...
with open(MODEL_DIR / "hls_light_cnn_80_180_summary.txt", "w") as f:
    model.summary(print_fn=lambda x: f.write(x + "\n"))


checkpoint = tf.keras.callbacks.ModelCheckpoint(
    MODEL_DIR / "hls_light_cnn_80_180_best_by_val_auc.h5",
    monitor="val_auc",
    mode="max",
    save_best_only=True,
    verbose=1,
...
print("\n[+] Training HLS-light CNN on 20x100 cropped input...\n")
...
model.save(MODEL_DIR / "hls_light_cnn_80_180_final.h5")
...
def plot_history_metric(key, ylabel, filename):
    ...
    plt.xlabel("Epoch")
    plt.ylabel(ylabel)
    plt.title(f"HLS-Light CNN 80-180 {ylabel}")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
...
all_weights_path = WEIGHT_DIR / "hls_light_cnn_80_180_all_weights.txt"

with open(all_weights_path, "w") as f_all:
    f_all.write("HLS-Light CNN 80-180 Weights\n")
    f_all.write("============================\n\n")
...
with open(MODEL_DIR / "final_summary.txt", "w") as f:
    ...
    f.write("=============\n\n")
    f.write("Model: HLS-Light CNN 80-180 crop\n")
